...
static void client_close(struct nvnc_client* client) {
  ...
  if (cleanup) cleanup(client->common.userdata);
  ...
  if (client->encoder) {
    client->server->n_damage_clients -=
        !(client->encoder->impl->flags & ENCODER_IMPL_FLAG_IGNORES_DAMAGE);
...
static inline void client_unref(struct nvnc_client* client) {
  ...
  if (--client->ref == 0) client_close(client);
}

static inline void client_ref(struct nvnc_client* client) { ++client->ref; }

static void deferred_client_close(void* obj) { client_unref(obj); }

static void stop_self(void* obj) { aml_stop(aml_get_default(), obj); }
...
static void close_after_write(void* userdata, enum stream_req_status status) {
  ...
  struct aml_idle* idle =
      aml_idle_new(stop_self, client, deferred_client_close);
...
static int on_version_message(struct nvnc_client* client) {
  if (client->buffer_len - client->buffer_index < 12) return 0;
...
#ifdef ENABLE_TLS
  if (client->server->auth_fn) security.types[0] = RFB_SECURITY_TYPE_VENCRYPT;
#endif

  stream_write(client->net_stream, &security, sizeof(security), NULL, NULL);
...
static int security_handshake_ok(struct nvnc_client* client) {
  ...
  return stream_write(client->net_stream, &result, sizeof(result), NULL, NULL);
}
...
static int send_byte_and_close(struct nvnc_client* client, uint8_t value) {
  return stream_write(client->net_stream, &value, 1, close_after_write, client);
}
...
static int on_vencrypt_version_message(struct nvnc_client* client) {
  struct rfb_vencrypt_version_msg* msg =
      (struct rfb_vencrypt_version_msg*)&client
          ->msg_buffer[client->buffer_index];

  if (client->buffer_len - client->buffer_index < sizeof(*msg)) return 0;
  ...
  struct rfb_vencrypt_subtypes_msg result = {
      .n = 1,
  };
...
static int on_vencrypt_subtype_message(struct nvnc_client* client) {
  ...
  if (stream_upgrade_to_tls(client->net_stream, client->server->tls_creds) <
      0) {
    client->state = VNC_CLIENT_STATE_ERROR;
...
static int on_security_message(struct nvnc_client* client) {
  if (client->buffer_len - client->buffer_index < 1) return 0;
...
static void disconnect_all_other_clients(struct nvnc_client* client) {
  ...
  if (node != client) {
    nvnc_log(NVNC_LOG_DEBUG, "disconnect other client %p (ref %d)", node,
             node->ref);
...
static void send_server_init_message(struct nvnc_client* client) {
  ...
  if (!display) {
    nvnc_log(NVNC_LOG_WARNING,
             "Tried to send init message, but no display has been added");
    goto close;
  }

  if (!display->buffer) {
    nvnc_log(NVNC_LOG_WARNING,
             "Tried to send init message, but no framebuffers have been set");
    goto close;
  }

  uint16_t width = nvnc_fb_get_width(display->buffer);
  uint16_t height = nvnc_fb_get_height(display->buffer);
  // this can cause issues on nvidia so we will always pass something sane here
  uint32_t fourcc = nvnc_fb_get_fourcc_format(display->buffer);
  if (fourcc == DRM_FORMAT_BGR888) {
...
static int on_init_message(struct nvnc_client* client) {
  ...
  if (!shared_flag) disconnect_all_other_clients(client);
...
static int on_client_set_pixel_format(struct nvnc_client* client) {
  ...
  struct rfb_pixel_format* fmt =
      (struct rfb_pixel_format*)(client->msg_buffer + client->buffer_index + 4);
...
static void send_cursor_update(struct nvnc_client* client) {
  ...
  stream_send(client->net_stream, rcbuf_new(payload.data, payload.len), NULL,
              NULL);
}
...
static int send_pts_rect(struct nvnc_client* client, uint64_t pts) {
  if (!will_send_pts(client, pts)) return 0;
...
static void process_fb_update_requests(struct nvnc_client* client) {
  ...
  if (!server->display || !server->display->buffer) return;

  if (client->net_stream->state == STREAM_STATE_CLOSED) return;

  if (client->is_updating || client->n_pending_requests == 0) return;
  ...
  if (fb->width != client->known_width || fb->height != client->known_height) {
    send_desktop_resize(client, fb);

    if (--client->n_pending_requests <= 0) return;
  }
  ...
  if (!pixman_region_not_empty(&client->damage)) return;
  ...
  DTRACE_PROBE2(neatvnc, process_fb_update_requests__encode, client, fb->pts);
...
static int on_client_fb_update_request(struct nvnc_client* client) {
   ...
   */
  if (!incremental) {
    pixman_region_union_rect(&client->damage, &client->damage, x, y, width,
                             height);

    if (client->encoder) encoder_request_key_frame(client->encoder);
  }

  DTRACE_PROBE1(neatvnc, update_fb_request, client);

  nvnc_fb_req_fn fn = server->fb_req_fn;
  if (fn) fn(client, incremental, x, y, width, height);
...
static int on_client_key_event(struct nvnc_client* client) {
  ...
  if (fn) fn(client, keysym, !!down_flag);
...
static int on_client_qemu_key_event(struct nvnc_client* client) {
  ...
  if (!evdev_keycode) evdev_keycode = xt_keycode;

  nvnc_key_fn fn = server->key_code_fn;
  if (fn) fn(client, evdev_keycode, !!down_flag);

  return sizeof(*msg);
}

static int on_client_qemu_event(struct nvnc_client* client) {
  if (client->buffer_len - client->buffer_index < 2) return 0;
  ...
  nvnc_log(NVNC_LOG_WARNING,
           "Got uninterpretable qemu message from client: %p (ref %d)", client,
           client->ref);
  nvnc_client_close(client);
  return 0;
}
...
static int on_client_pointer_event(struct nvnc_client* client) {
  ...
  if (fn) fn(client, x, y, button_mask);
...
static int on_client_cut_text(struct nvnc_client* client) {
  struct rfb_cut_text_msg* msg =
      (struct rfb_cut_text_msg*)(client->msg_buffer + client->buffer_index);

  size_t left_to_process = client->buffer_len - client->buffer_index;

  if (left_to_process < sizeof(*msg)) return 0;
  ...
  if (length > max_length) {
    nvnc_log(
        NVNC_LOG_ERROR,
        "Copied text length (%d) is greater than max supported length (%d)",
        length, max_length);
    nvnc_client_close(client);
    return 0;
  }
  ...
  if (msg_size <= left_to_process) {
    ...
    if (fn) fn(client, msg->text, length);
...
static void process_big_cut_text(struct nvnc_client* client) {
  ...
  if (n_read < 0) {
    if (errno != EAGAIN) {
      nvnc_log(NVNC_LOG_INFO, "Client connection error: %p (ref %d)", client,
               client->ref);
      nvnc_client_close(client);
    }
  ...
  if (client->cut_text.index != client->cut_text.length) return;

  nvnc_cut_text_fn fn = client->server->cut_text_fn;
  if (fn) fn(client, client->cut_text.buffer, client->cut_text.length);
...
static enum rfb_resize_status check_desktop_layout(struct nvnc_client* client,
  ...
  layout =
      malloc(sizeof(*layout) + n_screens * sizeof(*layout->display_layouts));
  if (!layout) return RFB_RESIZE_STATUS_OUT_OF_RESOURCES;
  ...
  for (size_t i = 0; i < n_screens; ++i) {
    ...
    if (screen->id == 0) display->display = server->display;
  ...
  if (!server->desktop_layout_fn || !server->desktop_layout_fn(client, layout))
...
static int on_client_set_desktop_size_event(struct nvnc_client* client) {
  ...
  msg = (struct rfb_client_set_desktop_size_event_msg*)(client->msg_buffer +
                                                        client->buffer_index);

  width = ntohs(msg->width);
  height = ntohs(msg->height);

  status = check_desktop_layout(client, width, height, msg->number_of_screens,
                                msg->screens);

  send_extended_desktop_size(client, RFB_RESIZE_INITIATOR_THIS_CLIENT, status);
...
static void update_ntp_stats(struct nvnc_client* client,
  ...
  nvnc_log(NVNC_LOG_DEBUG, "NTP: delta: %.2f ms, theta: %.2f ms", delta / 1e3,
           theta / 1e3);
}
...
static int on_client_message(struct nvnc_client* client) {
  ...
  nvnc_log(NVNC_LOG_WARNING,
           "Got uninterpretable message from client: %p (ref %d)", client,
           client->ref);
  nvnc_client_close(client);
  return 0;
}
...
static void record_peer_hostname(int fd, struct nvnc_client* client) {
  ...
  if (getpeername(fd, peer, &peerlen) == 0) {
    if (peer->sa_family == AF_UNIX) {
      ...
      getnameinfo(peer, peerlen, client->hostname, sizeof(client->hostname),
...
static struct nvnc* open_common(const char* address, uint16_t port,
  ...
  if (self->fd < 0) goto bind_failure;

  if (listen(self->fd, 16) < 0) goto listen_failure;

  self->poll_handle = aml_handler_new(self->fd, on_connection, self, NULL);
  if (!self->poll_handle) goto handle_failure;
...
void nvnc_close(struct nvnc* self) {
  ...
  if (cleanup) cleanup(self->common.userdata);

  if (self->display) nvnc_display_unref(self->display);

  if (self->cursor.buffer) nvnc_fb_unref(self->cursor.buffer);
  ...
  for (size_t i = 0; i < client->n_encodings; ++i)
    switch (client->encodings[i]) {
      ...
      case RFB_ENCODING_OPEN_H264:
        ...
        if (fb->type != NVNC_FB_GBM_BO) break;
        return client->encodings[i];
#endif
      default:
        break;
...
static bool client_has_encoding(const struct nvnc_client* client,
                                enum rfb_encodings encoding) {
  for (size_t i = 0; i < client->n_encodings; ++i)
    if (client->encodings[i] == encoding) return true;
...
static void finish_fb_update(struct nvnc_client* client, struct rcbuf* payload,
  ...
  if (client->net_stream->state == STREAM_STATE_CLOSED) goto complete;
  ...
  if (stream_write(client->net_stream, &update_msg, sizeof(update_msg), NULL,
                   NULL) < 0)
    goto complete;

  if (send_pts_rect(client, pts) < 0) goto complete;

  rcbuf_ref(payload);
  if (stream_send(client->net_stream, payload, on_write_frame_done, client) < 0)
    goto complete;
...
static int send_desktop_resize(struct nvnc_client* client, struct nvnc_fb* fb) {
  if (!client_has_encoding(client, RFB_ENCODING_DESKTOPSIZE) &&
    ...
    nvnc_log(NVNC_LOG_ERROR,
             "Client does not support desktop resizing. Closing connection...");
    nvnc_client_close(client);
    return -1;
  }
  ...
  if (client->encoder) encoder_resize(client->encoder, fb->width, fb->height);

  pixman_region_union_rect(&client->damage, &client->damage, 0, 0, fb->width,
                           fb->height);

  if (client_has_encoding(client, RFB_ENCODING_EXTENDEDDESKTOPSIZE)) {
    send_extended_desktop_size(client, RFB_RESIZE_INITIATOR_SERVER,
...
EXPORT
void nvnc_set_key_fn(struct nvnc* self, nvnc_key_fn fn) { self->key_fn = fn; }
...
void nvnc_add_display(struct nvnc* self, struct nvnc_display* display) {
  if (self->display) {
    nvnc_log(NVNC_LOG_PANIC,
             "Multiple displays are not implemented. Aborting!");
  }
...
void nvnc_remove_display(struct nvnc* self, struct nvnc_display* display) {
  if (self->display != display) return;
...
bool nvnc_client_supports_cursor(const struct nvnc_client* client) {
  for (size_t i = 0; i < client->n_encodings; ++i) {
    if (client->encodings[i] == RFB_ENCODING_CURSOR) return true;
  }
  return false;
}
...
int nvnc_enable_auth(struct nvnc* self, const char* privkey_path,
  ...
  rc = gnutls_certificate_set_x509_key_file(self->tls_creds, cert_path,
                                            privkey_path, GNUTLS_X509_FMT_PEM);
  if (rc != GNUTLS_E_SUCCESS) {
