...
static int tight_init_zs_worker(struct tight_encoder* self, int index) {
  ...
  if (!self->zs_worker[index]) goto failure;
...
static int tight_encoder_resize(struct tight_encoder* self, uint32_t width,
  ...
  if (self->grid) free(self->grid);

  self->grid =
      calloc(self->grid_width * self->grid_height, sizeof(*self->grid));
...
static int tight_encoder_init(struct tight_encoder* self, uint32_t width,
  ...
  if (tight_encoder_resize(self, width, height) < 0) return -1;
...
static int tight_apply_damage(struct tight_encoder* self,
  ...
  nvnc_log(NVNC_LOG_INFO, "damaged: %ld", n_damaged);
...
static void tight_encode_size(struct vec* dst, size_t size) {
  ...
  if (size >= 16384) vec_fast_append_8(dst, (size >> 14) & 0xff);
}

static int tight_deflate(struct tight_tile* tile, void* src, size_t len,
                         z_stream* zs, bool flush) {
  zs->next_in = src;
  zs->avail_in = len;

  do {
    if (tile->size >= MAX_TILE_SIZE) return -1;
...
static void tight_encode_tile_basic(struct tight_encoder* self,
                                    struct tight_tile* tile, uint32_t x,
                                    uint32_t y_start, uint32_t width,
                                    uint32_t height, int zs_index) {
  ...
  if (src_bpp == 4) {
    ...
    for (uint32_t y = y_start; y < y_start + height; ++y) {
      uint32_t* img = addr + x + y * stride;
      pixel32_to_cpixel(row, &cfmt, img, &self->sfmt, bytes_per_cpixel, width);

      // TODO What to do if the buffer fills up?
      if (tight_deflate(tile, row, bytes_per_cpixel * width, zs,
                        y == y_start + height - 1) < 0)
        abort();
    }
  } else if (src_bpp == 3) {
    ...
    for (uint32_t y = y_start; y < y_start + height; ++y) {
      ...
      pixel24_to_cpixel(row, &cfmt, img, &self->sfmt, bytes_per_cpixel, width);
...
static int tight_encode_tile_jpeg(struct tight_encoder* self,
  ...
  if (tjfmt == TJPF_UNKNOWN) return -1;
  ...
  int rc = -1;
  rc = tjCompress2(handle, img, width, stride, height, tjfmt, &buffer, &size,
                   subsampling, quality, TJFLAG_FASTDCT);
  if (rc < 0) {
...
static void tight_encode_tile(struct tight_encoder* self, uint32_t gx,
                              uint32_t gy) {
...
static int tight_schedule_encoding_jobs(struct tight_encoder* self) {
  for (int i = 0; i < 4; ++i)
    if (tight_schedule_zs_work(self, i) < 0) return -1;

  return 0;
}

static void tight_finish_tile(struct tight_encoder* self, uint32_t gx,
                              uint32_t gy) {
  struct tight_tile* tile = tight_tile(self, gx, gy);
  ...
  encode_rect_head(&self->dst, RFB_ENCODING_TIGHT, x_pos + x, y_pos + y, width,
                   height);
...
static int schedule_tight_finish(struct tight_encoder* self) {
  ...
  struct aml_work* work =
      aml_work_new(do_tight_finish, on_tight_finished, self, NULL);
  if (!work) {
    encoder_unref(&self->encoder);
    return -1;
  }
...
static void tight_encoder_set_output_format(
    struct encoder* encoder, const struct rfb_pixel_format* pixfmt) {
